...
result = NBufferCopyToPtr(hBuffer,bytebuff,sisa[0]);
	...
	char buffer[465]; // Buffer to store data
  	FILE * stream;
  	stream = fopen("b.dat", "r");
  	int count = fread(&buffer, 1, 465, stream);
  	fclose(stream);
  	int aaa= 0;
  	int iguales = 0;
	/*for(aaa=0;aaa<=464;aaa++){
		if(buffer[aaa]==bytebuff[aaa]){
			printf("[TRUE]\n");
			//printf("%d == %d [TRUE]\n", buffer[aaa],bytebuff[aaa]);
			iguales++;
		}
		else{
			printf("[FALSE]\n");
    ...
    printf("\n\n bytebuff[8] = %x %x %x %x %x %x %x %x %x %x\n\n",(unsigned char)bytebuff[0],(unsigned char)bytebuff[1],(unsigned char)bytebuff[2],(unsigned char)bytebuff[3],(unsigned char)bytebuff[4],(unsigned char)bytebuff[5],(unsigned char)bytebuff[6],(unsigned char)bytebuff[7],(unsigned char)bytebuff[8],(unsigned char)bytebuff[9]);
    ...
    result = NBufferCreateFromPtr(buffer,sisa[0],FALSE,&hBufferr);
	if (NFailed(result))
	{
	...
	int i;
    FILE *ptr;
	unsigned char buffer2[464];
	ptr = fopen("CPP.dat","rb");
    const size_t fileSize = fread(buffer2, sizeof(unsigned char), 464, ptr);
    printf("File size = %d bytes\n", fileSize);
    printf("Size of each item in bytes = %d\n", sizeof(unsigned char));
    for(i = 0; i </*464*/ (fileSize / sizeof(unsigned char)); i++)
    {
        printf("%x %x \n", (int)buffer2[i], (unsigned char)buffer[i]);
    }
    fclose (ptr);
	...
	result = NBufferCopyFromPtr(hBuffer,buffer2,sisa[0],0);
	if (NFailed(result)){ printf(N_T("MALDICION0 \n")); goto FINALLY; }

	result = NSubjectSetTemplateBuffer(hSubject, hBuffer);
	//result = NSubjectSetTemplateBuffer(hSubject, hBuffer);
	if (NFailed(result)){ printf(N_T("MALDICION1 \n")); goto FINALLY; }
	result = NSubjectSetId(hSubject, fileName);
	if (NFailed(result)){ printf(N_T("MALDICION2 \n")); goto FINALLY; }
